...
def create_time_file(time):
    if time == "reset":
        os.remove(PATH_TXT)
        print("The time has reset to the original system date")
    elif time in "1234567890":
        with open(PATH_TXT, "w") as write_file:
            write_file.write(time)
        now = date.today() + timedelta(days=int(int(time)))
        print(f"The new system time is {now}")
    else:
...
def parser_function():
    ...
    subparser = parser.add_subparsers(
        dest='command', description="Choose from the diferent departments [Buy , Sell , Stock , Search_Report, Advance_time]")
    ...
    buy_parser.add_argument(
        '-e', '--Expiration',  required=True, help="Expiration date on the Product e. g (yyyy-mm-dd)")
    ...
    search_group.add_argument('-from', '--Start',
                                help="Type in the dates you would like to see your inventory report (supported formats .e .g yyyy-mm-dd, now, yesterday, 3days, 5weeks 2months) ")
    ...
    #########################################################################
    advance_time_parser.add_argument('-a', '--Advance', help="You can change the current date, use this flag -a , to simulate action in the future, use the number of days in the future that you would like to change the current date in the program , or use the, reset, keyword to reset to original time .e.g -a 3 or -a reset ")
